...
-- ─────────────────────────────────────────────────────────────────────────────
-- 0. Auto-create profile on auth.users INSERT
--    The app passes handle + display_name as user metadata at sign-up time.
--    This SECURITY DEFINER trigger reads that metadata and creates the profile
--    row immediately — bypassing RLS — so the insert works whether or not
--    Supabase email confirmation is enabled (session may be null on sign-up).
--
--    Apple Sign-In: same trigger fires when the OAuth user row is created.
--    The app passes givenName/familyName as metadata via signInWithIdToken options.
-- ─────────────────────────────────────────────────────────────────────────────
...
--    Created automatically by the on_auth_user_created trigger above.
