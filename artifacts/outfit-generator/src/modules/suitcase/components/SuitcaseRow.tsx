/**
 * SuitcaseRow — fixed 3-slot carousel for the wardrobe closet view.
 *
...
import { type ClothingItem } from "../hooks/useLocalWardrobe";
import { getImageUrl } from "@/lib/utils";

// ── Types ─────────────────────────────────────────────────────────────────────
export interface SuitcaseRowHandle {
  scrollToIndex: (index: number, smooth?: boolean) => void;
  getLength: () => number;
}

interface SuitcaseRowProps {
  items: ClothingItem[];
...
// ── Component ─────────────────────────────────────────────────────────────────
export const SuitcaseRow = forwardRef<SuitcaseRowHandle, SuitcaseRowProps>(
        ...
        >
          {items.slice(lo, hi + 1).map((item, localIdx) => {
            ...
            if (transitioning) {
              ...
              bg      = `rgba(42,18,6,${(p * 0.88).toFixed(3)})`;
              shadow  = p > 0.05
...
SuitcaseRow.displayName = "SuitcaseRow";
