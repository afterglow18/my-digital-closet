/**
 * HandbagsRow — fixed 3-slot carousel for the wardrobe closet view.
 *
...
// ── Types ─────────────────────────────────────────────────────────────────────
export interface HandbagsRowHandle {
  scrollToIndex: (index: number, smooth?: boolean) => void;
  getLength: () => number;
}

interface HandbagsRowProps {
  items: ClothingItem[];
...
// ── Component ─────────────────────────────────────────────────────────────────
export const HandbagsRow = forwardRef<HandbagsRowHandle, HandbagsRowProps>(
        ...
        >
          {items.slice(lo, hi + 1).map((item, localIdx) => {
            ...
            if (transitioning) {
              ...
              bg      = p > 0.5 ? `rgba(78, 10, 20, ${((p - 0.5) * 2).toFixed(3)})` : "transparent";
              shadow  = p > 0.05
                ? `0 ${(4 * p).toFixed(1)}px ${(16 * p).toFixed(1)}px rgba(78,10,20,${(0.40 * p).toFixed(3)})`
                : "none";
            }
...
HandbagsRow.displayName = "HandbagsRow";
